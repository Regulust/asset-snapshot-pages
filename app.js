...
function loadState() {
  try {
    ...
    const seededDemo = seedRequestedDemoData(loaded);
    const seededTrend = seedAdditionalTrendSnapshots(loaded);
    const seededQuarterTrend = seedQuarterTrendTestSnapshots(loaded);
    const seededHistorical = seedHistoricalTrendTestSnapshots(loaded);
    const seededTags = seedSnapshotTagExamples(loaded);
    if (needsSignMigration || needsArchiveMigration || seededDemo || seededTrend || seededQuarterTrend || seededHistorical || seededTags) localStorage.setItem(STORAGE_KEY, JSON.stringify(loaded));
    return loaded;
  } catch {
...
function seedRequestedDemoData(targetState) {
  if (targetState.settings.demoSeedVersion >= 1) return false;
  const groups = targetState.settings.accountTypeGroups || [];
  const existingLatest = [...targetState.snapshots].sort((a, b) => b.date.localeCompare(a.date))[0];
  ...
  const sampleAccounts = groups.map((group, index) => {
    const type = group.types[index % Math.max(group.types.length, 1)]?.id || group.types[0]?.id;
    const name = `${group.name}示例账户`;
    let account = targetState.accounts.find((item) => item.name === name);
    if (!account && type) {
      account = {
        id: id(),
        name,
        type,
        group: group.name,
        currency: targetState.settings.baseCurrency,
        includeInNetWorth: true,
        archived: false,
      };
      targetState.accounts.push(account);
    }
    return account;
  });
  ...
  dates.forEach((date, dateIndex) => {
    let snapshot = targetState.snapshots.find((item) => item.date === date);
    if (!snapshot) {
      const balances = {};
      targetState.accounts.forEach((account) => {
        if (sampleAccounts.includes(account)) return;
        const latestValue = Number(existingLatest?.balances?.[account.id] || 0);
        balances[account.id] = Number((latestValue * (0.97 + dateIndex * 0.015)).toFixed(2));
      });
      snapshot = { id: id(), date, balances, rates: { ...targetState.settings.rates } };
      targetState.snapshots.push(snapshot);
    }
    sampleAccounts.forEach((account, groupIndex) => {
      if (account && snapshot.balances[account.id] === undefined) {
        snapshot.balances[account.id] = sampleBalances[groupIndex]?.[dateIndex] ?? (groupIndex + 1) * 1000;
      }
    });
  });
  targetState.settings.demoSeedVersion = 1;
  return true;
}

function seedAdditionalTrendSnapshots(targetState) {
  if (targetState.settings.demoTrendSeedVersion >= 1) return false;
  const reference = [...targetState.snapshots].sort((a, b) => b.date.localeCompare(a.date))[0];
  if (!reference) return false;
  const factors = [0.82, 0.85, 0.83, 0.88, 0.91, 0.89, 0.94, 0.92, 0.96, 0.95];
  const today = new Date();
  factors.forEach((factor, index) => {
    const daysAgo = 12 - index;
    const date = new Date(today.getFullYear(), today.getMonth(), today.getDate() - daysAgo);
    const dateText = localDateString(date);
    if (targetState.snapshots.some((snapshot) => snapshot.date === dateText)) return;
    const balances = {};
    targetState.accounts.forEach((account, accountIndex) => {
      const referenceValue = Number(reference.balances?.[account.id] || 0);
      const accountVariation = 1 + (((accountIndex % 3) - 1) * 0.006 * (index % 4));
      balances[account.id] = Number((referenceValue * factor * accountVariation).toFixed(2));
    });
    targetState.snapshots.push({
      id: id(),
      date: dateText,
      balances,
      rates: { ...targetState.settings.rates },
    });
  });
  targetState.settings.demoTrendSeedVersion = 1;
  return true;
}

function seedQuarterTrendTestSnapshots(targetState) {
  if (targetState.settings.demoQuarterTrendSeedVersion >= 1) return false;
  const reference = [...targetState.snapshots].sort((a, b) => b.date.localeCompare(a.date))[0];
  if (!reference) return false;
  ...
  samples.forEach(([monthDay, factor], index) => {
    const dateText = `${year}-${monthDay}`;
    if (targetState.snapshots.some((snapshot) => snapshot.date === dateText)) return;
    const balances = {};
    targetState.accounts.forEach((account, accountIndex) => {
      const referenceValue = Number(reference.balances?.[account.id] || 0);
      const wave = 1 + (((accountIndex % 4) - 1.5) * 0.004 * ((index % 3) + 1));
      balances[account.id] = Number((referenceValue * factor * wave).toFixed(2));
    });
    targetState.snapshots.push({
      id: id(),
      date: dateText,
  ...
  targetState.settings.demoQuarterTrendSeedVersion = 1;
  return true;
}

function seedHistoricalTrendTestSnapshots(targetState) {
  if (targetState.settings.demoHistoricalTrendSeedVersion >= 1) return false;
  const reference = [...targetState.snapshots].sort((a, b) => b.date.localeCompare(a.date))[0];
  if (!reference) return false;
  const samples = [
    ["2025-08-12", 0.52],
  ...
  samples.forEach(([dateText, factor], index) => {
    if (targetState.snapshots.some((snapshot) => snapshot.date === dateText)) return;
    const balances = {};
    targetState.accounts.forEach((account, accountIndex) => {
      const referenceValue = Number(reference.balances?.[account.id] || 0);
      const wave = 1 + (((accountIndex % 5) - 2) * 0.003 * ((index % 3) + 1));
      balances[account.id] = Number((referenceValue * factor * wave).toFixed(2));
    });
    targetState.snapshots.push({
      id: id(),
  ...
  targetState.settings.demoHistoricalTrendSeedVersion = 1;
  return true;
}

function seedSnapshotTagExamples(targetState) {
  if (targetState.settings.snapshotTagSeedVersion >= 1) return false;
  const examples = [
    ["工资收入"],
    ["奖金发放"],
    ["投资上涨"],
  ...
  [...targetState.snapshots]
    .sort((a, b) => b.date.localeCompare(a.date))
    .slice(0, examples.length)
    .forEach((snapshot, index) => {
      if (!Array.isArray(snapshot.tags) || snapshot.tags.length === 0) snapshot.tags = examples[index];
      if (snapshot.note === undefined) snapshot.note = "";
    });
  targetState.settings.snapshotTagSeedVersion = 1;
  return true;
}
...
function baselineCostsForDate(date) {
  ...
  state.accounts.forEach((account) => {
    if (costs[account.id] !== undefined) return;
    const cost = optionalNumber(account.costBasis);
    if (cost !== null) costs[account.id] = cost;
  });
  return costs;
}
